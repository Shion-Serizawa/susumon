...
<div class="mx-auto max-w-3xl">
  ...
  <section class="rounded-lg border border-gray-200 bg-white p-4">
    <div class="mb-3 flex items-center justify-between gap-3">
      <h2 class="text-base font-medium">Recent logs</h2>
      <a href="/app/logs" class="text-sm text-blue-700 hover:underline">View all</a>
    </div>

    {#if logsSource === "unavailable"}
      <div class="mb-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
        Logs API is not available yet. This section will populate once backend logs endpoints are implemented.
      </div>
    {/if}

    {#if recentLogs.length === 0}
      <div class="text-sm text-gray-600">No logs yet.</div>
    {:else}
        ...
        {#each recentLogs as log (log.id)}
          <li class="py-3">
            <div class="flex items-center justify-between gap-3">
              ...
              <a href={`/app/logs/${log.id}`} class="text-sm text-blue-700 hover:underline">Open</a>
            </div>
  ...
  <div class="mt-6">
    <a href="/app/themes" class="text-sm text-gray-700 hover:underline">← Back to themes</a>
  </div>
</div>
